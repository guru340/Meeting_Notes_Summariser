...
@Slf4j
@Component
public class SystemPromptAdvisior implements CallAdvisor, StreamAdvisor {
    @Override
    ...
    private ChatClientRequest updateSystemMessage(ChatClientRequest chatClientRequest) {
        List<Message> existingMessage=new ArrayList<>((Collection) chatClientRequest.prompt().getUserMessage());
        ...
        existingMessage.add(new SystemMessage(""" 
                        ...
                        4. Preserve important names, dates, numbers, and deadlines when provided. 5. If a section has no relevant information, write "Not specified". """+systemMessage));
